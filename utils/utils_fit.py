...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, 
                  gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0, tokenizer=None):
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, texts = batch
        #  test #
        texts_pre_encoded = tokenize(tokenizer, texts, truncate=True)
        with torch.no_grad():
            if cuda:
        ...
        if not fp16:
            ...
            logits_per_image, _                 = model_train(images, texts_pre_encoded)
