...
class CLIP(nn.Module):
    ...
    def encode_text(self, text_pre_encoded):
        if self.bert_type == "openai":
            text_pre_encoded = text_pre_encoded.to(self.visual.conv1.weight.device)
            x = self.token_embedding(text_pre_encoded).type(self.dtype)  # [batch_size, n_ctx, d_model]
            ...
            x = x[torch.arange(x.shape[0]), text_pre_encoded.argmax(dim=-1)] @ self.text_projection

        elif self.bert_type == "huggingface":
            x = self.tokenizer(text_pre_encoded, return_tensors="pt", padding=True)
            input_ids       = x.input_ids.to(self.visual.conv1.weight.device)
            attention_mask  = x.attention_mask.to(self.visual.conv1.weight.device)
            token_type_ids  = x.token_type_ids.to(self.visual.conv1.weight.device)
            x = self.transformer(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids).pooler_output
    ...
    def forward(self, image, text):
        image_features  = self.encode_image(image)     # image_features.shape like (16, 512)  (B, embed_dim)
        text_features   = self.encode_text(text)       # text_features.shape like (16, 512)  (B, transformer_width)
